#pylint: disable=W0511,C0103,W0621,E1101,W0212,W0703,R0912,R0914,R0915,R0911,W0105,C0324
""" Utility Functions """
...
def getUidGid(uname):
    ''' find uid, gid given a user name
...
def rchown(path, uid, gid):
    ...
    if os.path.exists(path):
        cmd = sudoCmd('chown -R %s:%s %s' % (str(uid), str(gid), path))
        ret = os.system(cmd)
        if (ret != 0):
...
def encrpyKey(script, pubkey, cleardata):
    ''' encrypt clear data with a given public key using openssl '''
    import subprocess
    proc = subprocess.Popen([script, pubkey, cleardata], stdout=subprocess.PIPE)
...
def trackable():
    ...
    def newf(f, self, *args, **kw):
        ...
        try:
            ...
            reqStr = contextutils.getcontext(self, 'reqstr')
            resMsg = contextutils.popcontext(self, 'resmsg')
            remoteAddr = contextutils.popcontext(self, 'remote_addr')

            #construct message
            msg.append('%s - %s' % ('correlationid', xstr(corId)))
            msg.append('%s - %s' % ('request', xstr(reqStr)))
            msg.append('%s - %s' % ('response', xstr(resMsg)))
...
def writeFile(filepath, content):
    """ write content to file as cronus user """
    try:
        with open(filepath, 'wb+') as fp:
            fp.write(content)
    except Exception as exc:
        LOG.error('Unable to write to file %s, %s' % (filepath, exc))
        raise exc
    
def dotNotation(obj, prefix=''):
    """ convert a nested json structure to flat use dot notation
        e.g. ['a', 'b.c', 'd[0].e', 'd[0].f', 'd[1].e', 'd[1].f']
    """
    if isinstance(obj, dict):
        if prefix: 
            prefix += '.'
        for k, v in obj.items():
            for res in dotNotation(v, prefix+str(k)):
                yield res
    elif isinstance(obj, list):
        for i, v in enumerate(obj):
            for res in dotNotation(v, prefix+'['+str(i)+']'):
                yield res
    else:
        yield prefix
