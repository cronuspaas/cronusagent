...
from agent.lib.errors import AgentException, Errors, ManifestNotFoundError
...
# additional logger for service logs
LOGHDLRS = {}
...
def updateServiceCatMeta(service, cat, pushedData=None):
    """ update particular category (section) within the metadata file """
    nPushedData = {cat: pushedData}
    return updateServiceMetaFile(service, nPushedData)
...
def readNestedServiceMeta(service, keyChain, defVal = None):
    """ get service meta value from keyChain of dot notation format
        @param service: service name
        @param keyChain: dot notation nested key chain
    ...
    return result if result else defVal   
...
def packagePaths(service, manifest):
    """
...
def getPackageScriptPath(service, manifest, package, script):
    """ return the full path to a script in cronus/scripts folder """
    manPath = manifestPath(service, manifest)
    path = os.path.join(manPath, package, 'cronus', 'scripts', script)
    if os.path.exists(path):
        return path
    else:
        return None
    
def getActiveScriptPath(service, package, script):
    """ return the full path to a script in cronus/scripts folder """
    manPath = manifestPath(service)
    path = os.path.join(manPath, package, 'cronus', 'scripts', script)
    if os.path.exists(path):
        return path
    else:
...
def getPackageFullLocByName(service, manifest, pkgnamePrefix):
    ...
    """
    if not service:
        raise AgentException(Errors.MANIFEST_MISSING_SERVICE, 'missing service')
...
class PkgInitConfig:
    ...
    def getConfig(self, cfgKey, defVal = None):
        """ get config value of a key """
        return self.cfg[cfgKey] if cfgKey in self.cfg else defVal
